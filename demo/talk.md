...
int main()
{
	const char* gid[] = { "tt011244f8", "tt0xx244f8" };
	...
	if (clientMgr)
	{
		...
		if (client)
		{
			...
			if (channel)
			{
				re = channel->setMetaData("tt011244f8", "admin", "admin", 0, 0, 2);
				re = channel->start();

				getchar();

				channel->startTalking();

				getchar();

				channel->sendAudioData(...);//正常是要等到DataSourceListener.onTalkingResp()返回正确后才能调用，此处是演示用
